...
typedef struct e_style {
  s21_size_t power;
  s21_size_t space_after;
  s21_size_t space_before;
  s21_size_t zero_before;
  bool sign_exp;
  bool sign_num;
} e_style;

typedef struct {
  s21_size_t count_of_spaces;
  s21_size_t count_of_zeros;
} int_style;

spec_params_t spec_parser(const char **buf);
...
void u_itoa(unsigned n, char s[]);
void lu_itoa(unsigned long, char s[]);
...
#define S21_ERRORS_COUNT 133
#define S21_ERRORS_LIST                                    \
  {                                                        \
      "Success",                                           \
